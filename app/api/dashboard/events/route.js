...
// GET all events with RSVP counts and whether current member is attending
export async function GET() {
  ...
  // Attach RSVP data to each event
  const enriched = events.map((ev) => {
    const evRsvps = rsvps.filter(r => r.event_id === ev.id);
    return {
      ...ev,
      rsvp_count: evRsvps.length,
      attending: evRsvps.some(r => r.member_id === member.id),
      attendees: evRsvps.map(r => r.member_name).filter(Boolean),
    };
  });
...
// POST create event (with optional flyer upload handled separately)
export async function POST(req) {
  ...
  if (contentType.includes('multipart/form-data')) {
    ...
    const up = await fetch(`${S}/storage/v1/object/dashboard/${filename}`, {
      method: 'POST',
      headers: { Authorization: `Bearer ${K}`, 'Content-Type': file.type },
      body: buf,
    });
...
// DELETE event (only creator or founders)
