...
export async function POST(req) {
  try {
    ...
    if (!fratName) {
      return Response.json({ success: false, message: 'Please enter your Big Brother name.' }, { status: 400 });
    }

    const supabase = createClient(SUPABASE_URL, SUPABASE_KEY, { db: { schema: 'members' } });

    // Find member by frat_name (case-insensitive)
    ...
    // Verify password
    if (!password) {
      return Response.json({ success: false, message: 'Please enter your password.' }, { status: 400 });
    }
